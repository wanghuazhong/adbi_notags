...
import temporary.stattaobaotag.AdCommonStatMapper;
import temporary.stattaobaotag.AdCommonStatReducer;

public class AdCommonStatJob implements Tool  {
	
	final String JOB_NAME = "ADBI_AdCommonStatJobTaobaoTagUV";
	...
	private void postProcessArgs() throws IOException {
		...
		if(fileStat.isDir()) {
			...
			for(FileStatus sub : subs) {
				if(!sub.getPath().getName().contains("step2")) {
					continue;
				}
