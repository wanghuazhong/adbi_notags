...
public class UARateStatJob implements Tool  {
	
	final String JOB_NAME = "ADBI_UARateStatJob";
	...
	private static final Log LOG = LogFactory.getLog(UARateStatJob.class);
	
	public UARateStatJob() {
		setupOptions();
	...
	public static void main(String[] args) throws Exception {
		int ret = 0;
		UARateStatJob job = new UARateStatJob();
	...
	private int submitAndMonitorJob() throws IOException {
		...
		//MapReduce class
		job.setMapperClass(UARateStep1Mapper.class);
		job.setReducerClass(UARateStep1Reducer.class);
	...
	private void setJobConf() throws IOException {
		...
		config_.set("mapreduce.lib.table.input.projection", UARateStep1Mapper.getProjectionConf());
