...
public class UARateStatJoinJob implements Tool  {
	
	final String JOB_NAME = "ADBI_UARateStatJoinJob";
	...
	private static final Log LOG = LogFactory.getLog(UARateStatJoinJob.class);
	
	public UARateStatJoinJob() {
		setupOptions();
	...
	public static void main(String[] args) throws Exception {
		int ret = 0;
		UARateStatJoinJob job = new UARateStatJoinJob();
	...
	private int submitAndMonitorJob() throws IOException {
		...
		//MapReduce class
		job.setMapperClass(UARateStep2Mapper.class);
		job.setReducerClass(UARateStep2Reducer.class);
