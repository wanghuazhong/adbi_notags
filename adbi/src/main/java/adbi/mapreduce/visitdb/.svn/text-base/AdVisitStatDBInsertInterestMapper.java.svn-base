...
public class AdVisitStatDBInsertInterestMapper extends MapReduceBase implements Mapper<LongWritable, Text, Text, Text>   {
	
	private final String LogAddr = "/user/aalog/ad_bi/log_tmp/AdVisitStatDBInsertInterestMapper/";
	...
	private StringBuffer sql_ad_interest_pv_click = new StringBuffer("insert into ad_interest_pv_click"
	...
	public void configure(JobConf context) {
		...
		LOG.info("AdDBInsertInterestMapper setup!");
	}
	...
	public void close() throws IOException {
		try {
			...
			LOG.info("AdDBInsertInterestMapper cleanup!");
			LOG.close();
		}
	}
	...
	public void map(LongWritable key, Text value,
		...
		try {
			...
			if(tag.length != 6)
			{
				LOG.warn("Input line format error, key is not 6 fileds(statindex,statdim,adkey,interest,adpos,adtype). key:" + splits[0]);
				return;
			}
			...
			if(tag.length != 2)
			{
				LOG.warn("Input line format error, value is not two fields(pv,click). value:" + splits[1]);
				return;
			}
