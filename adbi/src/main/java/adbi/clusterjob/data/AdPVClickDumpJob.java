...
import adbi.mapreduce.data.AdPVClickDumpMapper;
...
public class AdPVClickDumpJob implements Tool {

	String JOB_NAME = "ADBI_AdPVClickLogDumpJob_withOutTags_";
	...
	private static final Log LOG = LogFactory.getLog(AdPVClickDumpJob.class);
	
	public AdPVClickDumpJob() {
		setupOptions();
	...
	public static void main(String[] args) throws Exception {
		int ret = 0;
		AdPVClickDumpJob job = new AdPVClickDumpJob();
	...
	private int submitAndMonitorJob() throws IOException {
		...
		//MapReduce class
		job.setMapperClass(AdPVClickDumpMapper.class);
	...
	private void setJobConf() throws IOException {
		...
		LOG.debug("Zebero projection:" + AdPVClickDumpMapper.getProjectionConf());
		...
		config_.set("mapreduce.lib.table.input.projection", AdPVClickDumpMapper.getProjectionConf());
	...
	private void postProcessArgs() throws IOException {
		...
		if(fileStat.isDir()) {
			...
			for(FileStatus sub : subs) {
				if(!sub.getPath().getName().contains("SUB")) {
					continue;
				}
