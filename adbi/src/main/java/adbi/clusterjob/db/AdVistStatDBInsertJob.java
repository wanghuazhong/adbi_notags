...
import adbi.mapreduce.visitdb.AdVisitStatDBInsertInterestMapper;
import adbi.mapreduce.visitdb.AdVisitStatDBInsertRegionMapper;
import adbi.mapreduce.visitdb.AdVisitStatDBInsertTimeMapper;
import adbi.mapreduce.visitdb.AdVisitStatDBInsertUVMapper;

@SuppressWarnings("deprecation")
public class AdVistStatDBInsertJob extends Configured implements Tool {
	
	final String JOB_NAME = "ADBI_AdVistStatDBInsertJob_withOutTags_"
...
//	private String interestInput_;
	...
	private static final Log LOG = LogFactory.getLog(AdVistStatDBInsertJob.class);
	
	public AdVistStatDBInsertJob() {
		setupOptions();
	...
	public static void main(String[] args) throws Exception {
		int ret = 0;
		AdVistStatDBInsertJob job = new AdVistStatDBInsertJob();
	...
	private int submitAndMonitorJob() throws IOException {
        ...
        MultipleInputs.addInputPath(config_, new Path(timeInput_), TextInputFormat.class, AdVisitStatDBInsertTimeMapper.class);
        MultipleInputs.addInputPath(config_, new Path(regionInput_), TextInputFormat.class, AdVisitStatDBInsertRegionMapper.class);
//        MultipleInputs.addInputPath(config_, new Path(interestInput_), TextInputFormat.class, AdVisitStatDBInsertInterestMapper.class);
        MultipleInputs.addInputPath(config_, new Path(uvInput_), TextInputFormat.class, AdVisitStatDBInsertUVMapper.class);
...
//		Option interestInput = createOption("interestInput", "DFS input file(s) for the Map step", "path", Integer.MAX_VALUE, true);
	...
	public void exitUsage() {
		...
		System.out.println("  -interestInput <path> DFS input file(s) for the Map step");
...
//			interestInput_ = (String) cmdLine.getOptionValue("interestInput");
...
//				|| StringUtil.isBlank(interestInput_) 
...
//									"interestInput=["+ interestInput_ + "]\n" +
		...
		//process input3
//		if(interestInput_.endsWith("/")) {
//			interestInput_ = interestInput_.substring(0, interestInput_.length() - 1);
//		}
//		
//		//interestInput_ += "/" + timeStr;
//		fileStat = null;
//		try {
//			fileStat = client.getFileStatus(new Path(interestInput_));
//		} catch (Exception e) {
//			LOG.error("Get file status error:" + e.getMessage());
//			e.printStackTrace();
//		}
//		
//		if(fileStat == null) {
//			message = "path : " + interestInput_ + " not found";
//			fail(message);
//		}
//		LOG.info("interestInput:" + interestInput_);
//		
		//process input4
