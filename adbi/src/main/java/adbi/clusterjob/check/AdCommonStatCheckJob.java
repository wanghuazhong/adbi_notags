...
import adbi.mapreduce.check.AdCommonStatCheckMapper;
import adbi.mapreduce.check.AdCommonStatCheckReducer;

public class AdCommonStatCheckJob implements Tool {

	

	String JOB_NAME = "ADBI_AdCommonStatCheckJob_withOutTags_";
	final String Check_DIM = "Visit/PVClick/Time";
	...
	private static final Log LOG = LogFactory
			.getLog(AdCommonStatCheckJob.class);

	public AdCommonStatCheckJob() {
		setupOptions();
	...
	public static void main(String[] args) throws Exception {
		int ret = 0;
		AdCommonStatCheckJob job = new AdCommonStatCheckJob();
	...
	private int submitAndMonitorJob() throws IOException {
		...
		// MapReduce class
		job.setMapperClass(AdCommonStatCheckMapper.class);
		job.setReducerClass(AdCommonStatCheckReducer.class);
	...
	private void setupOptions() {
		Option time = createOption("time", "DFS input time for the Map step",
				"yyyy/MM/dd", Integer.MAX_VALUE, true);
		...
		Option libjars = createOption("lib", "sessionlog mapreduce lib jars",
				"file", Integer.MAX_VALUE, true);
		Option numReduceTasks = createOption("numReduceTasks", "Optional.",
				"spec", 1, true);

		allOptions = new Options().addOption(time).addOption(input)
				.addOption(output).addOption(libjars).addOption(numReduceTasks);
	}

	public void exitUsage() {
		System.out
				.println("Usage: $HADOOP_HOME/bin/hadoop jar $SESSIIONLOG_HOME/sessionlog.jar [options]");
		System.out.println("Options:");
		System.out
				.println("  -time <yyyy/MM/dd> DFS input time for the Map step");
		System.out
		...
		System.out
				.println("  -numReduceTasks <int> adpvinsight job reduce number");
	}
	...
	private void postProcessArgs() throws IOException {
		...
		if (StringUtil.isBlank(timeStr) || StringUtil.isBlank(input_)
